# SEDET MOD MENU V2
### 逃离后室终极修改器 | 源码可用 | 非商业 | 极致体验

[![版本](https://img.shields.io/badge/版本-V2.0.0-brightgreen)]()
[![许可协议](https://img.shields.io/badge/协议-PolyForm%20Noncommercial%201.0.0-blueviolet)]()
[![游戏](https://img.shields.io/badge/游戏-Escape%20The%20Backrooms-orange)]()

---

## ⚠️ 这不仅仅是个菜单

别再忍受枯燥的跑图和无聊的机制了。**SEDET MOD MENU V2** 是为《逃离后室》打造的终极控制台。无论是想做个快乐的探索者，还是成为后室的主宰，这玩意儿都能满足你。源码公开，免费使用，但别想拿去卖钱。

## 🚀 核心特性

*   **懒人狂喜的一键安装**：告别复杂的文件替换和配置修改。下载，注入，开玩。三岁小孩都能搞定。
*   **全能上帝模式**：
    *   **地图控制**：实时监控本地服务器状态，管理关卡流送与缓存。
    *   **Actor 操控**：生成、删除、随意摆弄场景里的任何物件。调整位置、大小、旋转，打造你的专属关卡。
    *   **玩家强化**：飞行、灵魂出窍、无限体力、坐标保存。想去哪就去哪，实体根本摸不到你。
*   **硬核防作弊系统**：内置反作弊机制，过滤恶意Mod，保护你的游戏环境不被捣乱。
*   **现代化 UI 设计**：深色主题 + 霓虹点缀。界面清晰，分类明确，功能再多也不迷路。
*   **自动更新检测**：每次启动自动检查最新版本，永远快人一步。

## 📸 界面预览

我们不整虚的，看图说话：

| 主菜单概览 | Actor 管理 |
| :---: | :---: |
| ![主菜单](pasted_image_1773574831445.png) | ![Actor管理](pasted_image_1773574844147.png) |
| **功能设置** | **玩家属性** |
| ![功能设置](pasted_image_1773574864927.png) | ![玩家属性](pasted_image_1773574875773.png) |

*(更多截图请移步网站(尚未开始运行))*

## 🎮 快速上手

1.  **下载**：去 [Releases](#) 页面下载最新版本。
2.  **放置**：解压文件到游戏目录或指定文件夹。
3.  **启动**：运行注入程序或启动游戏。
4.  **呼出**：游戏中按 **`F4`** 呼出菜单。

### 默认快捷键
*   `F4` - 打开/关闭菜单
*   `R` - 切换飞行模式
*   `Num -` - 销毁当前 Actor
*   `鼠标点击` - 选择功能

## ⚖️ 许可协议说明

本项目采用 **PolyForm Noncommercial License 1.0.0** 协议发布。

**简单来说：**
*   ✅ **允许**：个人学习、研究、非商业用途的自由使用和修改。
*   ❌ **禁止**：任何形式的商业用途（包括但不限于出售、集成到付费软件、利用本代码盈利）。
*   🛑 **限制**：你不得将本项目的代码闭源或更改为其他协议。

详情请阅读下方的 LICENSE 文件内容。

## ⚡ 免责声明

本软件仅供学习交流与非娱乐用途。请在单人模式或允许 MOD 的服务器中使用。滥用导致的封号或其他后果，开发者概不负责。用脑子玩游戏，别搞事。

---

**SteamID**: 76561199689713391 | **问题反馈**: Issues | **讨论**: Discussions
